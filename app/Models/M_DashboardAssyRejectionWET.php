...
class M_DashboardAssyRejectionWET extends Model
{
    ...
    public function get_data_total_reject_line_by_month($month) 
    {
        ...
        $query = $this->db->query('SELECT line, total_reject, total_aktual
                                    ...
                                    FROM lhp_produksi2
                                    WHERE MONTH(tanggal_produksi) = '.$bulan.' AND YEAR(tanggal_produksi) = '.$year.'
                                        AND (line = 8 OR line = 9)
                                    GROUP BY lhp_produksi2.line
                                    ) AS subquery
                                    WHERE line = 8 OR line = 9
                                    ORDER BY reject_percentage DESC
                                ');
    ...
    public function get_detail_summary_rejection_by_month($jenis_reject, $month, $line)
    {
        ...
        $year = date('Y', strtotime($month));
        if ($line == null || $line == 0) {
            $query = $this->db->query('SELECT MONTH(lhp_produksi2.tanggal_produksi) as month, detail_reject.kategori_reject, detail_reject.type_battery,master_pic_line.nama_pic, lhp_produksi2.shift, SUM(detail_reject.qty_reject) as qty
                                        FROM detail_reject
                                        JOIN lhp_produksi2 on lhp_produksi2.id_lhp_2 = detail_reject.id_lhp
                                        JOIN master_pic_line on master_pic_line.id_pic = lhp_produksi2.grup
                                        WHERE MONTH(lhp_produksi2.tanggal_produksi) = '.$bulan.' AND YEAR(lhp_produksi2.tanggal_produksi) = '.$year.'
                                        ...
                                        AND lhp_produksi2.line >= 8 AND lhp_produksi2.line <= 9
                                        AND detail_reject.jenis_reject != \'SETTING \'
                                        GROUP BY detail_reject.type_battery, detail_reject.kategori_reject, detail_reject.jenis_reject, MONTH(lhp_produksi2.tanggal_produksi),master_pic_line.nama_pic, lhp_produksi2.shift
                                        ORDER BY detail_reject.kategori_reject ASC, SUM(detail_reject.qty_reject) DESC
                                    ');
        } else {
            $query = $this->db->query('SELECT MONTH(lhp_produksi2.tanggal_produksi) as month, detail_reject.kategori_reject, detail_reject.type_battery,master_pic_line.nama_pic, lhp_produksi2.shift, SUM(detail_reject.qty_reject) as qty
    ...
    public function get_data_line_by_grup_month($month, $bulan, $line, $grup)
    {
        $year = date('Y', strtotime($month));
        if($line == 0) {
            $query = $this->db->query('SELECT MONTH(lhp_produksi2.tanggal_produksi), lhp_produksi2.line, master_pic_line.nama_pic, SUM(lhp_produksi2.total_aktual) AS total_aktual, SUM(detail_reject.qty_reject) AS qty_reject
                                        FROM		detail_reject
                                        JOIN        lhp_produksi2 ON lhp_produksi2.id_lhp_2 = detail_reject.id_lhp
                                        JOIN        master_pic_line ON master_pic_line.id_pic = lhp_produksi2.grup
    ...
    {
        if($line == 0) {
            $query = $this->db->query('SELECT lhp_produksi2.tanggal_produksi, lhp_produksi2.line, master_pic_line.nama_pic, SUM(lhp_produksi2.total_aktual) AS total_aktual, SUM(detail_reject.qty_reject) AS qty_reject
                                        FROM		detail_reject
                                        JOIN        lhp_produksi2 ON lhp_produksi2.id_lhp_2 = detail_reject.id_lhp
                                        JOIN        master_pic_line ON master_pic_line.id_pic = lhp_produksi2.grup
