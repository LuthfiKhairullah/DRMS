...
class M_LineStopWET extends Model
{
  ...
  {
    $query = $this->db->query('SELECT * FROM data_breakdown WHERE WET=\'1\'');
  ...
  {
    $query = $this->db->query('SELECT DISTINCT jenis_breakdown FROM data_breakdown WHERE WET=\'1\'');
  ...
  {
    $query = $this->db->query('SELECT DISTINCT dept_in_charge FROM data_breakdown WHERE WET=\'1\'');
  ...
  {
    $query = $this->db->query('SELECT DISTINCT perhitungan FROM data_breakdown WHERE WET=\'1\'');
