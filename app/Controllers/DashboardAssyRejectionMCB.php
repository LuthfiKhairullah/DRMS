...
use App\Models\M_DashboardAssyRejectionMCB;

class DashboardAssyRejectionMCB extends BaseController
{
    public function __construct()
    {
        $this->M_DashboardAssyRejectionMCB = new M_DashboardAssyRejectionMCB();
    ...
    public function dashboard_reject_assy()
    {
        ...
        $target = $this->M_DashboardAssyRejectionMCB->get_data_target_by_year('rejection', 'mcb', $bulan);
        if(count($target) > 0) {
            $data['target'] = $target[0]['target'];
        } else {
            $target = $this->M_DashboardAssyRejectionMCB->get_data_target_last_year('rejection', 'mcb', $bulan);
            if(count($target) > 0) {
                $data['target'] = $target[0]['target'];
            } else {
                $target = $this->M_DashboardAssyRejectionMCB->get_data_target_first_year('rejection', 'mcb', $bulan);
                if(count($target) > 0) {
                    $data['target'] = $target[0]['target'];
                } else {
                    $data['target'] = 0;
        ...
        // $total_data_reject_by_month = $this->M_DashboardAssyRejectionMCB->get_total_data_reject_by_month(idate('m',strtotime($bulan)));
        // $total_aktual_by_month = $this->M_DashboardAssyRejectionMCB->get_total_aktual_by_month(idate('m',strtotime($bulan)));
        $data_reject_by_month = $this->M_DashboardAssyRejectionMCB->get_data_reject_by_month($bulan, $child_filter);
        ...
        $data_jenis_reject = $this->M_DashboardAssyRejectionMCB->get_jenis_reject_by_month($start, $child_filter);

        if (!empty($data_jenis_reject)) {
            while (strtotime($start) <= strtotime($now)) {
                foreach ($data_jenis_reject as $d_jenis_reject) {
                    $data_jenis_reject_by_date = $this->M_DashboardAssyRejectionMCB->get_data_reject_by_date($start, $d_jenis_reject['jenis_reject'], $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardAssyRejectionMCB->get_data_rejection_by_month($bulan, $i, $child_filter);
            if (!empty($data_all)) {
        ...
        $data_line = $this->M_DashboardAssyRejectionMCB->get_data_total_reject_line_by_month($bulan);

        foreach ($data_line as $d_line) {
            array_push($data['data_reject_by_line'], 'MCB');
        ...
        $data_year = $this->M_DashboardAssyRejectionMCB->get_year_to_date_rejection($bulan, $child_filter);
        ...
        while (strtotime($start1) <= strtotime($now)) {
            $data_average_reject_by_date_all_line = $this->M_DashboardAssyRejectionMCB->get_data_reject_all_line_by_date($start1, $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardAssyRejectionMCB->get_data_average_reject_by_month($bulan, $i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_reject = $d_all['total_reject'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        // GET DATA AVERAGE REJECT BY DATE ALL LINE
        $data['data_reject_line_10'] = [];

        while (strtotime($start2) <= strtotime($now)) {
            for ($i=10; $i <= 10; $i++) { 
                $data_reject_per_line_by_date = $this->M_DashboardAssyRejectionMCB->get_data_reject_all_line($start2, $i);
        ...
        // GET DATA AVERAGE REJECT BY MONTH ALL LINE
        $data['data_reject_by_month_line_10'] = [];

        for ($h=1; $h <= 12; $h++) { 
            for ($i=10; $i <= 10; $i++) {
                $data_reject_all_line_by_month = $this->M_DashboardAssyRejectionMCB->get_data_reject_all_line_by_month($bulan, $h, $i);
        ...
        $data_jenis_reject_persentase = $this->M_DashboardAssyRejectionMCB->get_jenis_reject_by_month($start3, $child_filter);
        ...
        if (!empty($data_jenis_reject_persentase)) {
            while (strtotime($start3) <= strtotime($now)) {
                foreach ($data_jenis_reject_persentase as $d_jenis_reject) {
                    $data_jenis_reject_by_date_persentase = $this->M_DashboardAssyRejectionMCB->get_data_reject_by_date($start3, $d_jenis_reject['jenis_reject'], $child_filter);
                    $data_reject_per_line_by_date = $this->M_DashboardAssyRejectionMCB->get_data_reject_all_line($start3, $child_filter);
        ...
        return view('dashboard/dashboard_lhp_assy_rejection_mcb', $data);
    }
    ...
    public function get_detail_rejection()
    {
        ...
        $data['total_aktual_by_month'] = $this->M_DashboardAssyRejectionMCB->get_total_aktual_by_month($date, $line);
        $data['total_aktual_by_date'] = $this->M_DashboardAssyRejectionMCB->get_total_aktual_by_date($date, $line);

        $data['data_jenis_reject'] = $this->M_DashboardAssyRejectionMCB->get_qty_jenis_reject($date, $line);
        $data['data_jenis_reject_by_month'] = $this->M_DashboardAssyRejectionMCB->get_data_reject_by_month($date, $line);

        $data['data_reject_by_jenis_reject'] = $this->M_DashboardAssyRejectionMCB->get_detail_rejection_by_jenis($jenis_reject, $date, $line);
        $data['data_reject_by_type_battery'] = $this->M_DashboardAssyRejectionMCB->get_detail_rejection_by_type_battery($jenis_reject, $date, $line);
        $data['data_reject_by_grup'] = $this->M_DashboardAssyRejectionMCB->get_detail_rejection_by_grup($jenis_reject, $date, $line);

        $data['data_jenis_reject_by_type_battery'] = $this->M_DashboardAssyRejectionMCB->get_jenis_reject_by_type_battery($type_battery, $date, $line);
        $data['data_kategori_reject_by_type_battery'] = $this->M_DashboardAssyRejectionMCB->get_kategori_reject_by_type_battery($type_battery, $date, $line);
        $data['data_grup_reject_by_type_battery'] = $this->M_DashboardAssyRejectionMCB->get_grup_reject_by_type_battery($type_battery, $date, $line);

        $data['data_jenis_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionMCB->get_jenis_reject_by_grup_shift($grup, $shift, $date, $line);
        $data['data_kategori_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionMCB->get_kategori_reject_by_grup_shift($grup, $shift, $date, $line);
        $data['data_battery_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionMCB->get_battery_reject_by_grup_shift($grup, $shift, $date, $line);

        $data['data_all_detail_kategori_rejection_by_date'] = $this->M_DashboardAssyRejectionMCB->get_all_detail_kategori_rejection_by_date($date, $line);
        $data['data_all_detail_battery_rejection_by_date'] = $this->M_DashboardAssyRejectionMCB->get_all_detail_battery_rejection_by_date($date, $line);
        $data['data_all_detail_grup_rejection_by_date'] = $this->M_DashboardAssyRejectionMCB->get_all_detail_grup_rejection_by_date($date, $line);

        $data['data_reject_by_jenis_reject_by_month'] = $this->M_DashboardAssyRejectionMCB->get_detail_rejection_by_jenis_by_month($jenis_reject, $date, $line);
        $data['data_reject_by_type_battery_by_month'] = $this->M_DashboardAssyRejectionMCB->get_detail_rejection_by_type_battery_by_month($jenis_reject, $date, $line);

        $data['data_all_detail_kategori_rejection_by_month'] = $this->M_DashboardAssyRejectionMCB->get_all_detail_kategori_rejection_by_month($date, $line);
        $data['data_all_detail_battery_rejection_by_month'] = $this->M_DashboardAssyRejectionMCB->get_all_detail_battery_rejection_by_month($date, $line);
        $data['data_all_detail_grup_rejection_by_month'] = $this->M_DashboardAssyRejectionMCB->get_all_detail_grup_rejection_by_month($date, $line);

        $data['detail_summary_rejection'] = $this->M_DashboardAssyRejectionMCB->get_detail_summary_rejection($jenis_reject, $date, $line);
    ...
    public function dashboard_reject_assy_qc()
    {
        ...
        $data_line_by_date = $this->M_DashboardAssyRejectionMCB->get_data_total_reject_line_by_date($bulan);
