...
class DashboardMCB extends BaseController
{
    ...
    public function dashboard_lhp_assy()
    {
        ...
        $target = $this->M_Dashboard->get_data_target_by_year('efficiency', 'mcb', $bulan);
        if(count($target) > 0) {
            $data['target'] = $target[0]['target'];
        } else {
            $target = $this->M_Dashboard->get_data_target_last_year('efficiency', 'mcb', $bulan);
            if(count($target) > 0) {
                $data['target'] = $target[0]['target'];
            } else {
                $target = $this->M_Dashboard->get_data_target_first_year('efficiency', 'mcb', $bulan);
                if(count($target) > 0) {
                    $data['target'] = $target[0]['target'];
                } else {
                    $data['target'] = 0;
        ...
        $data['data_line_10'] = [];
        $data['data_line_10_pcs'] = [];

        $data['data_by_month_line_10'] = [];
        ...
        if ($jenis_dashboard == 1 AND ($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'average') {
            while (strtotime($start) <= strtotime($now)) {
                $data_all = $this->M_Dashboard->get_data_all_line_by_date_mcb($start);
                if (!empty($data_all)) {
            ...
            for ($h=1; $h <= 12; $h++) { 
                for ($i=10; $i <= 10; $i++) {
                    $data_all_line_by_month = $this->M_Dashboard->get_data_all_line_by_month_mcb($bulan, $h, $i);
        ...
        $data['data_line_10_current_month'] = [];

        //PUSH DATA PER LINE UNTUK BULAN BERJALAN
        for ($i=10; $i <= 10; $i++) { 
            $data_all = $this->M_Dashboard->get_data_all_line_by_month_mcb($bulan, $current_date, $i);
            if (!empty($data_all)) {
        ...
        $data['data_line_10_previous_month'] = [];

        //PUSH DATA PER LINE UNTUK BULAN SEBELUMNYA
        for ($i=10; $i <= 10; $i++) { 
            $data_all = $this->M_Dashboard->get_data_all_line_by_month_mcb($bulan, $previous_date, $i);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_Dashboard->get_data_all_line_by_month_mcb($bulan, $i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        $data_year = $this->M_Dashboard->get_data_all_line_by_year_mcb($bulan, $child_filter);
        ...
        if(date('Y-m') == date('Y-m', strtotime($bulan))) {
            foreach ($arr_jam as $jam) {
                $data_all = $this->M_Dashboard->get_data_all_line_by_jam_mcb($child_filter, $jam, $tanggal_berjalan);
                if (!empty($data_all)) {
                    foreach($data_all as $d_all) {
                        $total_plan = $d_all['total_plan'];
                        $total_aktual = $d_all['total_aktual'];
        ...
        $data_top_grup_current_month = $this->M_Dashboard->get_data_top_grup_mcb($bulan, $current_date);
        ...
        $data_top_grup_daily = [];
        if(date('Y-m') == date('Y-m', strtotime($bulan))) {
            $data_top_grup_daily = $this->M_Dashboard->get_data_top_grup_mcb_daily(date('Y-m-d'));
        }
        $data['top_grup_daily'] = $data_top_grup_daily;

        
        return view('dashboard/dashboard_lhp_assy_mcb', $data);
    }
