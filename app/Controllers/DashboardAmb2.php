...
class DashboardAmb2 extends BaseController
{
    ...
    public function dashboard_lhp_assy()
    {
        ...
        if ($jenis_dashboard == 1 AND ($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'average') {
            ...
            for ($h=1; $h <= 12; $h++) { 
                for ($i=4; $i <= 7; $i++) {
                    $data_all_line_by_month = $this->M_Dashboard->get_data_all_line_by_month_amb2($bulan, $h, $i);
        ...
        for ($i=4; $i <= 7; $i++) { 
            $data_all = $this->M_Dashboard->get_data_all_line_by_month_amb2($bulan, $current_date, $i);
            if (!empty($data_all)) {
        ...
        for ($i=4; $i <= 7; $i++) { 
            $data_all = $this->M_Dashboard->get_data_all_line_by_month_amb2($previous_month, $previous_date, $i);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_Dashboard->get_data_all_line_by_month_amb2($bulan, $i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        $data_year = $this->M_Dashboard->get_data_all_line_by_year_amb2($bulan, $child_filter);
        ...
        $data_top_grup_current_month = $this->M_Dashboard->get_data_top_grup_amb2($bulan, $current_date);
        ...
        $data_top_grup_previous_month = $this->M_Dashboard->get_data_top_grup_amb2($bulan, $previous_date);
