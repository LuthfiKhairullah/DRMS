...
use App\Models\M_DashboardAssyRejectionWET;

class DashboardAssyRejectionWET extends BaseController
{
    public function __construct()
    {
        $this->M_DashboardAssyRejectionWET = new M_DashboardAssyRejectionWET();
    ...
    public function dashboard_reject_assy()
    {
        ...
        $target = $this->M_DashboardAssyRejectionWET->get_data_target_by_year('rejection', 'wet', $bulan);
        if(count($target) > 0) {
            $data['target'] = $target[0]['target'];
        } else {
            $target = $this->M_DashboardAssyRejectionWET->get_data_target_last_year('rejection', 'wet', $bulan);
            if(count($target) > 0) {
                $data['target'] = $target[0]['target'];
            } else {
                $target = $this->M_DashboardAssyRejectionWET->get_data_target_first_year('rejection', 'wet', $bulan);
                if(count($target) > 0) {
                    $data['target'] = $target[0]['target'];
                } else {
                    $data['target'] = 0;
        ...
        // $total_data_reject_by_month = $this->M_DashboardAssyRejectionWET->get_total_data_reject_by_month(idate('m',strtotime($bulan)));
        // $total_aktual_by_month = $this->M_DashboardAssyRejectionWET->get_total_aktual_by_month(idate('m',strtotime($bulan)));
        $data_reject_by_month = $this->M_DashboardAssyRejectionWET->get_data_reject_by_month($bulan, $child_filter);
        ...
        $data_jenis_reject = $this->M_DashboardAssyRejectionWET->get_jenis_reject_by_month($start, $child_filter);

        if (!empty($data_jenis_reject)) {
            while (strtotime($start) <= strtotime($now)) {
                foreach ($data_jenis_reject as $d_jenis_reject) {
                    $data_jenis_reject_by_date = $this->M_DashboardAssyRejectionWET->get_data_reject_by_date($start, $d_jenis_reject['jenis_reject'], $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardAssyRejectionWET->get_data_rejection_by_month($bulan, $i, $child_filter);
            if (!empty($data_all)) {
        ...
        $data_line = $this->M_DashboardAssyRejectionWET->get_data_total_reject_line_by_month($bulan);
        ...
        $data_year = $this->M_DashboardAssyRejectionWET->get_year_to_date_rejection($bulan, $child_filter);
        ...
        while (strtotime($start1) <= strtotime($now)) {
            $data_average_reject_by_date_all_line = $this->M_DashboardAssyRejectionWET->get_data_reject_all_line_by_date($start1, $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardAssyRejectionWET->get_data_average_reject_by_month($bulan, $i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_reject = $d_all['total_reject'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        if ($jenis_dashboard == 1 AND ($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'line') {
            ...
            while (strtotime($start2) <= strtotime($now)) {
                for ($i=8; $i <= 9; $i++) { 
                    $data_reject_per_line_by_date = $this->M_DashboardAssyRejectionWET->get_data_reject_all_line($start2, $i);
            ...
            for ($h=1; $h <= 12; $h++) { 
                for ($i=8; $i <= 9; $i++) {
                    $data_reject_all_line_by_month = $this->M_DashboardAssyRejectionWET->get_data_reject_all_line_by_month($bulan, $h, $i);
            ...
            $data_grup_line = $this->M_DashboardAssyRejectionWET->get_data_grup_by_line($bulan, $child_filter);

            $data_grup_line_year = $this->M_DashboardAssyRejectionWET->get_data_grup_by_line_year($bulan, $child_filter);

            if (!empty($data_grup_line_year)) {
                for ($i=1; $i <= 12 ; $i++) { 
                    $qty_reject_grup_month = 0;
                    $total_aktual_grup_month = 0;
                    $eff = 0;
                    foreach ($data_grup_line_year as $d_grup_line) {
                        $grup_month = $d_grup_line['nama_pic'];
                        $data_all_grup_month = $this->M_DashboardAssyRejectionWET->get_data_line_by_grup_month($bulan, $i, $child_filter, $grup_month);
                        ...
                        if (!empty($data_all_grup_month)) {
                            foreach ($data_all_grup_month as $d_all_grup) {
                                $qty_reject_grup_month += $d_all_grup['qty_reject'];
                        ...
                        $eff = (!empty($total_aktual_grup_month)) ? ($qty_reject_grup_month / $total_aktual_grup_month) * 100 : 0;
            ...
            if (!empty($data_grup_line)) {
                    while (strtotime($start) <= strtotime($now)) {
                        foreach ($data_grup_line as $d_grup_line) {
                            ...
                            $data_all_grup = $this->M_DashboardAssyRejectionWET->get_data_line_by_grup($start, $child_filter, $grup);
                            if (!empty($data_all_grup)) {
                                foreach ($data_all_grup as $d_all_grup) {
                                    $qty_reject_grup = $d_all_grup['qty_reject'];
                                    $total_aktual_grup = $d_all_grup['total_aktual'];
                                    $eff = (!empty($total_aktual_grup)) ? ($qty_reject_grup / $total_aktual_grup) * 100 : 0;
        ...
        $data_jenis_reject_persentase = $this->M_DashboardAssyRejectionWET->get_jenis_reject_by_month($start3, $child_filter);
        ...
        if (!empty($data_jenis_reject_persentase)) {
            while (strtotime($start3) <= strtotime($now)) {
                foreach ($data_jenis_reject_persentase as $d_jenis_reject) {
                    $data_jenis_reject_by_date_persentase = $this->M_DashboardAssyRejectionWET->get_data_reject_by_date($start3, $d_jenis_reject['jenis_reject'], $child_filter);
                    $data_reject_per_line_by_date = $this->M_DashboardAssyRejectionWET->get_data_reject_all_line($start3, $child_filter);
        ...
        return view('dashboard/dashboard_lhp_assy_rejection_wet', $data);
    }
    ...
    public function get_detail_rejection()
    {
        ...
        $data['total_aktual_by_month'] = $this->M_DashboardAssyRejectionWET->get_total_aktual_by_month($date, $line);
        $data['total_aktual_by_date'] = $this->M_DashboardAssyRejectionWET->get_total_aktual_by_date($date, $line);

        $data['data_jenis_reject'] = $this->M_DashboardAssyRejectionWET->get_qty_jenis_reject($date, $line);
        $data['data_jenis_reject_by_month'] = $this->M_DashboardAssyRejectionWET->get_data_reject_by_month($date, $line);

        $data['data_reject_by_jenis_reject'] = $this->M_DashboardAssyRejectionWET->get_detail_rejection_by_jenis($jenis_reject, $date, $line);
        $data['data_reject_by_type_battery'] = $this->M_DashboardAssyRejectionWET->get_detail_rejection_by_type_battery($jenis_reject, $date, $line);
        $data['data_reject_by_grup'] = $this->M_DashboardAssyRejectionWET->get_detail_rejection_by_grup($jenis_reject, $date, $line);

        $data['data_jenis_reject_by_type_battery'] = $this->M_DashboardAssyRejectionWET->get_jenis_reject_by_type_battery($type_battery, $date, $line);
        $data['data_kategori_reject_by_type_battery'] = $this->M_DashboardAssyRejectionWET->get_kategori_reject_by_type_battery($type_battery, $date, $line);
        $data['data_grup_reject_by_type_battery'] = $this->M_DashboardAssyRejectionWET->get_grup_reject_by_type_battery($type_battery, $date, $line);

        $data['data_jenis_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionWET->get_jenis_reject_by_grup_shift($grup, $shift, $date, $line);
        $data['data_kategori_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionWET->get_kategori_reject_by_grup_shift($grup, $shift, $date, $line);
        $data['data_battery_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionWET->get_battery_reject_by_grup_shift($grup, $shift, $date, $line);

        $data['data_all_detail_kategori_rejection_by_date'] = $this->M_DashboardAssyRejectionWET->get_all_detail_kategori_rejection_by_date($date, $line);
        $data['data_all_detail_battery_rejection_by_date'] = $this->M_DashboardAssyRejectionWET->get_all_detail_battery_rejection_by_date($date, $line);
        $data['data_all_detail_grup_rejection_by_date'] = $this->M_DashboardAssyRejectionWET->get_all_detail_grup_rejection_by_date($date, $line);

        $data['data_reject_by_jenis_reject_by_month'] = $this->M_DashboardAssyRejectionWET->get_detail_rejection_by_jenis_by_month($jenis_reject, $date, $line);
        $data['data_reject_by_type_battery_by_month'] = $this->M_DashboardAssyRejectionWET->get_detail_rejection_by_type_battery_by_month($jenis_reject, $date, $line);

        $data['data_all_detail_kategori_rejection_by_month'] = $this->M_DashboardAssyRejectionWET->get_all_detail_kategori_rejection_by_month($date, $line);
        $data['data_all_detail_battery_rejection_by_month'] = $this->M_DashboardAssyRejectionWET->get_all_detail_battery_rejection_by_month($date, $line);
        $data['data_all_detail_grup_rejection_by_month'] = $this->M_DashboardAssyRejectionWET->get_all_detail_grup_rejection_by_month($date, $line);

        $data['detail_summary_rejection'] = $this->M_DashboardAssyRejectionWET->get_detail_summary_rejection($jenis_reject, $date, $line);
        $data['detail_summary_rejection_by_month'] = $this->M_DashboardAssyRejectionWET->get_detail_summary_rejection_by_month($jenis_reject, $date, $line);
    ...
    public function dashboard_reject_assy_qc()
    {
        ...
        $data_line_by_date = $this->M_DashboardAssyRejectionWET->get_data_total_reject_line_by_date($bulan);
