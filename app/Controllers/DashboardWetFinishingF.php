...
class DashboardWetFinishingF extends BaseController
{
    ...
    public function dashboard_lhp_wet_finishing()
    {
        ...
        if ($jenis_dashboard == 1 AND ($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'average') {
            while (strtotime($start) <= strtotime($now)) {
                $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_date_wet_f($start);
                if (!empty($data_all)) {
            ...
            for ($h=1; $h <= 12; $h++) { 
                for ($i=9; $i <= 9; $i++) {
                    $data_all_line_by_month = $this->M_DashboardWetFinishing->get_data_all_line_by_month_wet_f($bulan, $h, $i);
        ...
        for ($i=9; $i <= 9; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month_wet_f($bulan, $current_date, $i);
            if (!empty($data_all)) {
        ...
        for ($i=9; $i <= 9; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month_wet_f($bulan, $previous_date, $i);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month_wet_f($bulan, $i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        $data_year = $this->M_DashboardWetFinishing->get_data_all_line_by_year_wet_f($bulan, $child_filter);
        ...
        foreach ($arr_jam as $jam) {
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_jam_wet_f($child_filter, $jam, $tanggal_berjalan);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        $data_top_grup_current_month = $this->M_DashboardWetFinishing->get_data_top_grup_wet_f($current_date);
        $data['top_grup_current_month'] = $data_top_grup_current_month;


        //PUSH TOP GRUP UNTUK BULAN BERJALAN
        $data_top_grup_daily = $this->M_DashboardWetFinishing->get_data_top_grup_wet_f_daily(date('Y-m-d'));
        $data['top_grup_daily'] = $data_top_grup_daily;

        
        return view('dashboard_wet_finishing/dashboard_lhp_wet_f', $data);
    }
