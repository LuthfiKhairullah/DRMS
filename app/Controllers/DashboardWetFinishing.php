...
use App\Models\M_DashboardWetFinishing;

class DashboardWetFinishing extends BaseController
{
    public function __construct()
    {
        $this->M_DashboardWetFinishing = new M_DashboardWetFinishing();
    ...
    public function dashboard_lhp_wet_finishing()
    {
        ...
        $target = $this->M_DashboardWetFinishing->get_data_target_by_year('efficiency', 'wet_finishing', $bulan);
        if(count($target) > 0) {
            $data['target'] = $target[0]['target'];
        } else {
            $target = $this->M_DashboardWetFinishing->get_data_target_last_year('efficiency', 'wet_finishing', $bulan);
            if(count($target) > 0) {
                $data['target'] = $target[0]['target'];
            } else {
                $target = $this->M_DashboardWetFinishing->get_data_target_first_year('efficiency', 'wet_finishing', $bulan);
                if(count($target) > 0) {
                    $data['target'] = $target[0]['target'];
                } else {
                    $data['target'] = 0;
        ...
        $data['data_line_8_pcs'] = [];
        $data['data_line_9_pcs'] = [];

        $data['data_by_month_line_8'] = [];
        $data['data_by_month_line_9'] = [];
        ...
        if ($jenis_dashboard == 1 AND ($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'average') {
            while (strtotime($start) <= strtotime($now)) {
                $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_date($start);
                if (!empty($data_all)) {
            ...
            for ($h=1; $h <= 12; $h++) { 
                for ($i=8; $i <= 9; $i++) {
                    $data_all_line_by_month = $this->M_DashboardWetFinishing->get_data_all_line_by_month($bulan, $h, $i);
            ...
            while (strtotime($start) <= strtotime($now)) {
                for ($i=8; $i <= 9; $i++) { 
                    $data1 = $this->M_DashboardWetFinishing->get_data_all_line($start, $i);
                    if (!empty($data1)) {
                        foreach ($data1 as $d1) {
        ...
        } elseif ($jenis_dashboard == 1 AND ($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'grup') {
            $data_grup_line = $this->M_DashboardWetFinishing->get_data_grup_by_line($start, '');

            $data_grup_line_year = $this->M_DashboardWetFinishing->get_data_grup_by_line_year($start, '');
            ...
            if (!empty($data_grup_line_year)) {
                for ($i=1; $i <= 12 ; $i++) { 
                    ...
                    foreach ($data_grup_line_year as $d_grup_line) {
                        ...
                        $data_all_grup_month = $this->M_DashboardWetFinishing->get_data_line_by_grup_month($bulan, $i, '', $grup_month);
            ...
            if (!empty($data_grup_line)) {
                while (strtotime($start) <= strtotime($now)) {
                    foreach ($data_grup_line as $d_grup_line) {
                        ...
                        $data_all_grup = $this->M_DashboardWetFinishing->get_data_line_by_grup($start, '', $grup);
            ...
            while (strtotime($start) <= strtotime($now)) {
                $data1 = $this->M_DashboardWetFinishing->get_data_all_line($start, $child_filter);
                if (!empty($data1)) {
                    foreach ($data1 as $d1) {
                        $total_plan = $d1['total_plan'];
                        $total_aktual = $d1['total_aktual'];
            ...
            for ($h=1; $h <= 12; $h++) { 
                for ($i=1; $i <= 3; $i++) {
                    $data_all_line_by_month_by_shift = $this->M_DashboardWetFinishing->get_data_all_line_by_month_by_shift($bulan, $h, $child_filter, $i);
            ...
            while (strtotime($start) <= strtotime($now)) {
                $data1 = $this->M_DashboardWetFinishing->get_data_line($start, $child_filter, 1);
                if (!empty($data1)) {
                    foreach ($data1 as $d1) {
                        $total_plan = $d1['total_plan'];
                        $total_aktual = $d1['total_aktual'];
                ...
                $data2 = $this->M_DashboardWetFinishing->get_data_line($start, $child_filter, 2);
                if (!empty($data2)) {
                    foreach ($data2 as $d2) {
                ...
                $data3 = $this->M_DashboardWetFinishing->get_data_line($start, $child_filter, 3);
                if (!empty($data3)) {
                    foreach ($data3 as $d3) {
            ...
            $data_grup_line = $this->M_DashboardWetFinishing->get_data_grup_by_line($start, $child_filter);

            $data_grup_line_year = $this->M_DashboardWetFinishing->get_data_grup_by_line_year($start, $child_filter);
            ...
            if (!empty($data_grup_line_year)) {
                for ($i=1; $i <= 12 ; $i++) { 
                    ...
                    foreach ($data_grup_line_year as $d_grup_line) {
                        $grup_month = $d_grup_line['nama_pic'];
                        $data_all_grup_month = $this->M_DashboardWetFinishing->get_data_line_by_grup_month($bulan, $i, $child_filter, $grup_month);
            ...
            if (!empty($data_grup_line)) {
                    while (strtotime($start) <= strtotime($now)) {
                        foreach ($data_grup_line as $d_grup_line) {
                            $grup = $d_grup_line['nama_pic'];
                            $data_all_grup = $this->M_DashboardWetFinishing->get_data_line_by_grup($start, $child_filter, $grup);
                            if (!empty($data_all_grup)) {
                                foreach ($data_all_grup as $d_all_grup) {
                                    $total_plan_grup = $d_all_grup['total_plan'];
                                    $total_aktual_grup = $d_all_grup['total_aktual'];
            ...
            $data_kss_line = $this->M_DashboardWetFinishing->get_data_kss_by_line($start, $child_filter);

            $data_kss_line_year = $this->M_DashboardWetFinishing->get_data_kss_by_line_year($start, $child_filter);
            ...
            if (!empty($data_kss_line_year)) {
                for ($i=1; $i <= 12 ; $i++) { 
                    ...
                    foreach ($data_kss_line_year as $d_kss_line) {
                        ...
                        $data_all_kss_month = $this->M_DashboardWetFinishing->get_data_line_by_kss_month($bulan, $i, $child_filter, $kss_month);
            ...
            if (!empty($data_kss_line)) {
                    while (strtotime($start) <= strtotime($now)) {
                        foreach ($data_kss_line as $d_kss_line) {
                            ...
                            $data_all_kss = $this->M_DashboardWetFinishing->get_data_line_by_kss($start, $child_filter, $kss);
        ...
        $data['data_line_8_current_month'] = [];
        $data['data_line_9_current_month'] = [];

        //PUSH DATA PER LINE UNTUK BULAN BERJALAN
        for ($i=8; $i <= 9; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month($bulan, $current_date, $i);
            if (!empty($data_all)) {
        ...
        $data['data_line_8_previous_month'] = [];
        $data['data_line_9_previous_month'] = [];

        //PUSH DATA PER LINE UNTUK BULAN SEBELUMNYA
        for ($i=8; $i <= 9; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month($previous_month, $previous_date, $i);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month($bulan, $i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        $data_year = $this->M_DashboardWetFinishing->get_data_all_line_by_year($bulan, $child_filter);
        ...
        if(date('Y-m') == date('Y-m', strtotime($bulan))) {
            foreach ($arr_jam as $jam) {
                $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_jam($child_filter, $jam, $tanggal_berjalan);
                if (!empty($data_all)) {
                    foreach($data_all as $d_all) {
                        $total_plan = $d_all['total_plan'];
                        $total_aktual = $d_all['total_aktual'];
        ...
        $data_top_grup_daily = $this->M_DashboardWetFinishing->get_data_top_grup_daily(date('Y-m-d'));
        $data['top_grup_daily'] = $data_top_grup_daily;

        //PUSH TOP GRUP UNTUK BULAN BERJALAN
        $data_top_grup_monthly = $this->M_DashboardWetFinishing->get_data_top_grup_monthly($bulan, $current_date);
        $data['top_grup_monthly'] = $data_top_grup_monthly;


        //PUSH TOP GRUP UNTUK TAHUN BERJALAN
        $data_top_grup_yearly = $this->M_DashboardWetFinishing->get_data_top_grup_yearly(date('Y', strtotime($bulan)));
        $data['top_grup_yearly'] = $data_top_grup_yearly;
        
        return view('dashboard_wet_finishing/dashboard_lhp_wet', $data);
    }
    ...
    public function get_data_line_stop() {
        ...
        $data = $this->M_DashboardWetFinishing->get_data_line_stop($tanggal, $line);
    ...
    public function get_data_line_stop_by_shift() {
        ...
        $data = $this->M_DashboardWetFinishing->get_data_line_stop_by_shift($tanggal, $line, $shift);
        echo json_encode($data);
    }
    ...
    public function get_data_line_stop_by_grup() {
        ...
        $data = $this->M_DashboardWetFinishing->get_data_line_stop_by_grup($tanggal, $line, $grup);
        echo json_encode($data);
    }
    ...
    public function get_data_line_stop_by_kss() {
        ...
        $data = $this->M_DashboardWetFinishing->get_data_line_stop_by_kss($tanggal, $line, $kss);
        echo json_encode($data);
    }
}