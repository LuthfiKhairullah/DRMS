...
if ($data_lhp[0]['line'] == 8) {
	$line = 11;
} elseif ($data_lhp[0]['line'] == 9) {
	$line = 12;
} else {
	$line = $data_lhp[0]['line'];
}
								...
								<div class="table-responsive">
									<table id="" class="table table-striped mb-0">
										...
										<tbody id="tbody">
											...
											// Initialize a variable to store the maximum batch value
											$maxBatch = 0;

											// Iterate through the array to find the maximum batch value
											foreach ($data_detail_lhp as $item) {
												if ($item['batch'] > $maxBatch) {
													$maxBatch = $item['batch'];
												}
											}

											if ($maxBatch <= count($jam_start)) {
												for ($i = $maxBatch; $i < count($jam_start); $i++) {
													$j = $i;
													$k = $i + 1 ?>
													<tr>
														<td>
															<button type="button" class="btn btn-sm btn-primary" onclick="add_rows_batch(<?= $k ?>)">
																Add
															</button>
														</td>
														<td>
															<div id="start_section_<?= $k ?>">
																<input type="time" class="form-control" name="start[]" id="start_<?= $k ?>" value="<?= date('H:i', strtotime(str_replace('.', ':', $jam_start[$j]))) ?>" style="width: 100px;">
															</div>
														</td>
														<td>
															<div id="stop_section_<?= $k ?>">
																<input type="time" class="form-control" name="stop[]" id="stop_<?= $k ?>" value="<?= date('H:i', strtotime(str_replace('.', ':', $jam_end[$j]))) ?>" style="width: 100px;">
															</div>
														</td>
														<td>
															<input type="number" class="form-control" name="menit_terpakai[]" id="menit_terpakai_<?= $k ?>" value="<?= $menit_aktual[$j] ?>" onkeyup="update_plan(<?= $k ?>)" style="width: 100px">
														</td>
														<td>
												...
												<tr>
													...
													<td>
														<select class="form-control select2" id="kategori_pending_<?= $index_pending ?>" name="kategori_pending[]" style="width: 250px;" required>
															...
															<?php $data_kategori_pending = $this->M_WET_Finishing->getKategoriPending($d_detail_pending['jenis_pending']);
								...
								<div class="table-responsive">
									<table id="" class="table table-striped mb-0">
										...
										<tbody id="tbody_line_stop">
											<?php $model = new App\Models\M_Data();
											$index_breakdown = 0;
												...
												<tr>
													...
													<td>
														<select class="form-control select2" name="jenis_breakdown[]" id="jenis_breakdown_<?= $index_breakdown ?>" onchange="get_proses_breakdown(<?= $index_breakdown ?>)" style="width: 200px">
															...
															<?php foreach ($data_breakdown as $d_jenis_breakdown) { ?>
													...
													<td>
														<select class="form-control select2" id="proses_breakdown_<?= $index_breakdown ?>" name="proses_breakdown[]" style="width: 250px;">
															...
															<?php $data_proses_breakdown = $model->getProsesBreakdown($d_detail_breakdown['jenis_breakdown']);
															$andon_proses = true;
												...
												<tr>
													...
													<td>
														<select class="form-control select2" name="jenis_reject[]" id="jenis_reject_<?= $index_reject ?>" onchange="get_kategori_reject(<?= $index_reject ?>)" style="width: 200px" required>
															...
															<?php foreach ($data_reject as $d_jenis_reject) { ?>
													...
													<td>
														<select class="form-control select2" id="kategori_reject_<?= $index_reject ?>" name="kategori_reject[]" style="width: 250px;" required>
															...
															<?php $data_kategori_reject = $model->getKategoriReject($d_detail_reject['jenis_reject']);
...
<script>
			...
			<tr>
				...
				<td>
					<select class="form-control select2" id="no_wo_${k}" name="no_wo[]" onchange="getPartNo(${k})" style="width: 200px;">
						<option selected disabled>-- Pilih No WO --</option>
						${data_wo.map(item => {
							if(item.PDNO.substring(0, 4) == 'PKLC') return `<option value="${item.PDNO}">${item.PDNO}</option>`
						})}
						<option value="-">-</option>
					</select>
	...
	function add_pending(i) {
		...
		var j = tbody.rows.length;
		if(no_wo_pending != '' && no_wo_pending != null) {
