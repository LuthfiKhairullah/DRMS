...
<?php
    ...
    if ($child_filter == 0 AND ($baby_filter == 'line' OR $baby_filter == 'grup')) {
        $type_chart = 'line';
    } else {
        $type_chart = 'column';
    }
                        ...
                        <div class="box-body" style="display:flex">
                            ...
                            <div class="col-6" style="display:flex; margin-top:40px;">
                                <div class="col-3" style="display:flex;text-align:center;flex-direction: column;align-items: center;flex-wrap: nowrap;justify-content: space-around; margin-left:-140px; margin-top:-65px;">
                                    <a href="<?=base_url()?>dashboard/wet_finishing" class="waves-effect waves-light btn btn-rounded btn-success btn-lg btn-nav">Efficiency</a>
...
<script>
    ...
    Highcharts.chart('side_chart', {
        ...
        legend: {
            <?php if (($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND ($baby_filter == 'line' OR $baby_filter == 'grup')) { ?>
                align: 'center',
        ...
        series: [
            ...
            <?php if (($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'line') { ?>
                {
                    name: 'Wet A',
                    data: <?php echo json_encode($data_by_month_line_8); ?>,
                },
                {
                    name: 'Wet F',
                    data: <?php echo json_encode($data_by_month_line_9); ?>,
                },
            <?php } else if (($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'grup') { 
    ...
    Highcharts.chart('main_chart', {
        ...
        series: [
                    ...
                    point: {
                        events: {
                            click: function() {
                                ...
                                $.ajax({
                                    url: "<?= base_url('dashboard/wet_finishing/get_data_line_stop'); ?>",
                                    type: "POST",
                                    data: {
                                        date: date,
                                    },
            ...
            <?php if (($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'line') { ?>
                {
                    name: 'Wet A',
                    data: <?php echo json_encode($data_line_8); ?>,
                    point: {
                        events: {
                            click: function() {
                                var date = $('#bulan').val()+'-'+this.category;
                ...
                {
                    name: 'Wet F',
                    data: <?php echo json_encode($data_line_9); ?>,
                    point: {
                        events: {
                            click: function() {
                                var date = $('#bulan').val()+'-'+this.category;
            ...
            <?php } ?>
            <?php if (($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'grup') { 
                        ...
                        point: {
                            events: {
                                click: function() {
                                    ...
                                    $.ajax({
                                        url: "<?= base_url('dashboard/wet_finishing/get_data_line_stop_by_grup'); ?>",
                                        type: "POST",
                                        data: {
                                            date: date,
                                            line: line,
                    ...
                    point: {
                        events: {
                            click: function() {
                                ...
                                var shift = 1;
                                $.ajax({
                                    url: "<?= base_url('dashboard/wet_finishing/get_data_line_stop_by_shift'); ?>",
                                    type: "POST",
                                    data: {
                                        date: date,
                                        line: line,
                ...
                foreach ($res_data_line_by_kss as $r_data_line_by_kss) { ?>
                    {
                        ...
                        events: {
                            click: function() {
                                ...
                                $.ajax({
                                    url: "<?= base_url('dashboard/wet_finishing/get_data_line_stop_by_kss'); ?>",
                                    type: "POST",
                                    data: {
                                        date: date,
                                        line: line,
