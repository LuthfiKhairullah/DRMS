            ...
            <div class="row">
                <div class="box bg-transparent">
                    <div class="box-body" style="display:flex">
                        <div class="col-2">
                            <form action="<?=base_url()?>dashboard/reject_mcb" method="POST">
                                ...
                                <select class="form-select" name="child_filter" id="child_filter" style="border-width: thick;border: wh;font-size: 20px;font-weight: 900;width: 175px;">
                                    ...
                                    <?php for ($i=10; $i <= 10 ; $i++) { ?>
                                        <option value="<?=$i?>" <?= ($child_filter == $i) ? 'selected':''?>><?= $i == 10 ? 'MCB' : 'Line ' . $i ?></option>
                                    <?php } ?>
                                </select>
                                &nbsp;
                        ...
                        <div class="col-6" style="display:flex; margin-top:35px;">
                            <div class="col-3" style="display:flex;text-align:center;flex-direction: column;align-items: center;flex-wrap: nowrap;justify-content: space-around; margin-left:-70px; margin-top:-65px;">
                                <a href="<?=base_url()?>dashboard/assy/mcb" class="waves-effect waves-light btn btn-outline btn-rounded btn-success btn-lg btn-nav">Efficiency</a>
                                <a href="<?=base_url()?>dashboard/reject_mcb" class="waves-effect waves-light btn btn-rounded btn-danger btn-lg btn-nav">Rejection</a>
                                <a href="<?=base_url()?>dashboard/line_stop_mcb" class="waves-effect waves-light btn btn-outline btn-rounded btn-warning btn-lg btn-nav">Line Stop</a>
...
<script>
    ...
    Highcharts.chart('pareto_reject', {
        ...
        },
        {
            type: 'spline',
            name: 'Target',
            data: [<?php foreach($data_total_reject_by_line as $dt) {
    ...
    <?php if($baby_filter == 'average') { ?>
        Highcharts.chart('average_daily_chart', {
            ...
            title: {
                text: 'Daily Rejection <?=($child_filter == 0) ? 'All Line' : ($child_filter == 10 ? 'MCB' : 'Line '.$child_filter)?>',
                align: 'center',
                style: {
                    color: '#ffffff',
                    fontSize: '20px'
                ...
                column: {
                    ...
                    events: {
                        click : function(e) {
                            ...
                            $.ajax({
                                url: '<?= base_url('dashboard/reject_mcb/get_detail_rejection') ?>',
                                type: 'POST',
                                data: {
                                    date: date,
                                    line: line
    ...
    <?php } elseif ($baby_filter == 'line') { ?>
        Highcharts.chart('average_daily_chart_by_line', {
            ...
            series: [
                {
                        name: 'MCB',
                        data: <?php echo json_encode($data_reject_line_10); ?>
                    },
            ],
        ...
        Highcharts.chart('average_month_chart_by_line', {
            ...
            series: [
                {
                    name: 'MCB',
                    data: <?php echo json_encode($data_reject_by_month_line_10); ?>,
                },
            ]
        });
    <?php    }   ?>
